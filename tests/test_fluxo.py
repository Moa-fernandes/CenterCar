...
def sessao_em_memoria(tmp_path, monkeypatch):
    ...
    sess.add_all([
        Veiculo(marca='X', modelo='Alpha', ano=2020, motorizacao='1.0',
                tipo_combustivel='Gasolina', cor='Branco', quilometragem=1000,
                numero_portas=4, transmissao='Manual', preco=50000),
        Veiculo(marca='Y', modelo='Beta', ano=2018, motorizacao='2.0',
                tipo_combustivel='Diesel', cor='Preto', quilometragem=2000,
                numero_portas=2, transmissao='Automática', preco=70000),
    ])
    sess.commit()
    yield sess
    sess.close()
